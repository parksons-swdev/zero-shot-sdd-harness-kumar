...
The CSV Insight Agent is a personal data-analysis assistant. The user uploads a CSV or Excel (`.xlsx`/`.xls`, first sheet) export from their own machine and asks natural-language questions about it. The agent answers with a plain-language summary containing key numbers, an interactive chart, and a summary table, and proactively flags anomalies or data-quality issues it notices (missing values, outliers, inconsistent formats) — without being asked. It supports both fast single-shot answers and multi-step reasoning where it writes and runs analysis code, observes the result, and iterates when needed.
...
### Phase 3 — Excel Upload + ChatGPT-Style UI & Theming

- **Goal:** the user can upload Excel workbooks (`.xlsx` / `.xls`) as well as CSVs and get the exact same profiling/analysis experience, and the whole app is redressed as a polished ChatGPT-style interface — a collapsible left sidebar listing past analyses (folding in History navigation), a centered conversation column with message bubbles, a question box pinned to the bottom, and a dark/light theme toggle that defaults to system preference. Every Phase 1/2 behavior remains working and reachable; this phase adds one backend input format and a visual/structural redesign, nothing else.
- **Independent slices (parallel build units):**
  - `excel-ingestion` (backend) — generalize the ingestion path to accept Excel. Add an Excel reader alongside `parse_csv` and a `parse_file(file_path)` dispatcher that routes by extension, reading `.xlsx` via pandas `read_excel(engine="openpyxl")` and legacy `.xls` via `read_excel(engine="xlrd")`, always using the **first sheet** of the workbook. It returns the **same** `ParsedCsv` / `ParseDecision` contract so all downstream profiling/anomaly/graph code is unchanged. Malformed/unreadable Excel produces the same structured `ParseDecision` (ask-the-user), never a crash. Update `POST /datasets` to accept the new extensions/content-types (keeping the 100MB limit and the same response envelope), update accepted-extension validation and error messages, and add `openpyxl` + `xlrd` to `pyproject.toml`. Deps: none (extends files this slice owns).
  - `chatgpt-redesign` (frontend) — the **entire** ChatGPT-style redesign as ONE cohesive slice so nothing collides on `page.tsx` / `layout.tsx`: collapsible left sidebar (past-analyses list folding in History nav, plus a link to the full `/app/history` screen and the "Recent Datasets" picker), centered conversation column with user/assistant message bubbles, question input pinned to the bottom, dark/light theme provider (Tailwind `dark` class strategy, default = `prefers-color-scheme`, choice persisted in `localStorage`), theme toggle, and theme-aware Plotly (dark template in dark mode). Every Phase 1/2 feature — upload (now incl. Excel), ask, step-progress, plain-language answer + key numbers + interactive chart + summary table + collapsible "View analysis code" + token/cost badge, anomaly flags, clarifying-question flow, stuck-point transparency, run history (search + date filters), Recent Datasets reselect — stays present, working, and reachable. All existing `data-testid` hooks and accessible names are preserved (see below). Deps: none.
  - `e2e-tests-phase3` (frontend + backend) — authored against the redesigned DOM: a Playwright spec covering Excel upload end-to-end and the theme toggle (light↔dark, persistence, dark Plotly template), plus reconciling `tests/e2e/phase1.spec.ts` and `tests/e2e/phase2.spec.ts` against the redesigned selectors **in lockstep** — preserving every existing `data-testid`/role or updating the spec's selectors to the new ones without deleting a test or weakening an assertion. Deps (to run, not to author): `excel-ingestion`, `chatgpt-redesign`.
- **Key surfaces / files:**
  - `excel-ingestion`: `src/tools/ingestion.py` (add `parse_excel` + `parse_file` dispatcher), `src/api/datasets.py` (accepted extensions/content-types + messages), `src/config/settings.py` (if an allowed-extensions setting exists), `pyproject.toml` (`openpyxl`, `xlrd`), `tests/unit/tools/test_ingestion.py`, `tests/fixtures/sample_sales.xlsx`, `tests/fixtures/sample_sales.xls`
  - `chatgpt-redesign`: `frontend/src/app/layout.tsx`, `frontend/src/app/page.tsx`, `frontend/src/app/history/page.tsx`, `frontend/src/components/Sidebar.tsx`, `frontend/src/components/ChatColumn.tsx`, `frontend/src/components/MessageBubble.tsx`, `frontend/src/components/ThemeProvider.tsx`, `frontend/src/components/ThemeToggle.tsx`, `frontend/src/components/Chart*.tsx` (theme-aware Plotly), `frontend/tailwind.config.*` (`darkMode: 'class'`)
  - `e2e-tests-phase3`: `tests/e2e/phase3.spec.ts`, `tests/e2e/phase1.spec.ts`, `tests/e2e/phase2.spec.ts` (selector reconciliation only)
- **Gate command:**
  1. `uv run pytest tests/unit -q` — includes `tests/unit/tools/test_ingestion.py`, which asserts: `parse_file` on a committed `.xlsx` and a committed `.xls` fixture (each the first sheet of a multi-sheet workbook) returns a `ParsedCsv` with the same schema/row/column contract as the equivalent CSV; a corrupt/unreadable Excel file returns a `ParseDecision` (never raises); and CSV parsing is unchanged. (Offline gate — no LLM/network required.)
  2. `cd frontend && pnpm build` — the redesigned static export builds; built CSS contains real Tailwind utility selectors including `dark:` variants.
  3. `uv run python -m src` — boots FastAPI serving the redesigned frontend at `http://localhost:8001/app/` with no import errors.
  4. `npx playwright test tests/e2e/ --reporter=line` — runs `phase1`, `phase2`, and `phase3` specs against the redesigned DOM: existing journeys still pass, Excel upload works end-to-end, and the theme toggle switches and persists.
  > **Real-LLM/e2e note:** gates 3–4 exercise the live Gemini API and may return HTTP 429 on the Gemini free-tier daily quota today — this is an environmental quota limit, not a code defect. Gates 1–2 are fully offline and are the authoritative code-correctness gates for this phase.
- **How the user tests it (handoff seed):**
  1. Run `cd frontend && pnpm build`, then from the repo root `uv run python -m src`; open `http://localhost:8001/app/`.
  2. Note the new ChatGPT-style layout: a collapsible left sidebar listing past analyses, a centered conversation column, and the question box pinned to the bottom. Toggle the sidebar collapsed/expanded.
  3. Upload an Excel file (`.xlsx` or `.xls`) — the committed fixtures at `tests/fixtures/sample_sales.xlsx` / `sample_sales.xls` work, or your own workbook (first sheet is used). Confirm it profiles and answers exactly like a CSV.
  4. Ask a question and confirm the full answer still renders: summary, key numbers, interactive chart, summary table, collapsible "View analysis code", token/cost badge, anomaly flags — and try an ambiguous question for the clarifying-question flow.
  5. Click the theme toggle to switch dark↔light; reload and confirm your choice persisted; in dark mode confirm the Plotly chart uses a dark template. On first visit (cleared localStorage) the theme follows your OS setting.
  6. Confirm History (via the sidebar and the full `/app/history` screen with search + date filters) and Recent Datasets reselect all still work.
