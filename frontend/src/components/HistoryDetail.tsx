...
function TableView({ rows }: { rows: Record<string, unknown>[] }) {
  if (!rows || rows.length === 0) {
    return <p className="text-sm text-gray-500">No table data for this run.</p>
  }
...
function ChartView({ spec }: { spec: ChartSpec }) {
  if (!spec || !spec.x || !spec.y) {
    return <p className="text-sm text-gray-500">No chart data for this run.</p>
  }
  const chartType = spec.type === 'line' ? 'scatter' : (spec.type ?? 'bar')
  return (
    <div className="rounded-lg border border-gray-200 p-2" data-testid="history-chart">
      <Plot
        data={[
          {
            x: spec.x,
      ...
      {!loading && !error && detail && (
        <div className="space-y-6" data-testid="history-detail">
          {detail.question_text && (
            <div>
              <p className="text-xs font-medium tracking-wide text-gray-400 uppercase">Question</p>
              <p className="text-base font-medium text-gray-900">{detail.question_text}</p>
            </div>
          )}
          ...
          {detail.status === 'completed' && (
            <>
              {detail.answer_text && (
                <div>
                  <p className="text-xs font-medium tracking-wide text-gray-400 uppercase">Answer</p>
                  <p className="whitespace-pre-wrap text-sm text-gray-800">{detail.answer_text}</p>
                </div>
              )}

              {detail.key_numbers && Object.keys(detail.key_numbers).length > 0 && (
                <div>
                  <p className="mb-2 text-xs font-medium tracking-wide text-gray-400 uppercase">Key numbers</p>
                  <div className="flex flex-wrap gap-3">
                    {Object.entries(detail.key_numbers).map(([label, value]) => (
                      <div key={label} className="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm">
              ...
              {detail.chart_spec && (
                <div>
                  <p className="mb-2 text-xs font-medium tracking-wide text-gray-400 uppercase">Chart</p>
                  <ChartView spec={detail.chart_spec} />
                </div>
              )}

              {detail.table_data && (
                <div>
                  <p className="mb-2 text-xs font-medium tracking-wide text-gray-400 uppercase">Table</p>
